...
routes.get("/throw-error", (req, res) => {
  ...
  throw new Error(
    "This is an intentional error which should be logged to stderr."
  );
});
...
routes.get("/:documentId", (req, res) => {
  ...
  const document = mockData.find(
    (doc) => doc.id === Number.parseInt(documentId, 10)
  );
  assert(
    document,
    new NotFoundError(`document with id ${documentId} does not exist`)
  );
...
routes.post("/", (req, res) => {
  ...
  assert(
    !mockData.some((existing) => existing.id === document.id),
    new ConflictError(`document with id ${document.id} already exists`)
  );
...
//       const existingItem = mockData.find(
